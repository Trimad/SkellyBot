...
class QuizMenu {
  ...
  static QuizSelector(msg, quizMenu, state) {
    const menuEmbed = {
      color: 0x0099ff,
    ...
    msg.channel.send({ embed: menuEmbed }).then(() => {
      ...
      msg.channel.awaitMessages(filter, { max: 1, time: 10000, errors: ['time'] })
        .then(collected => {
          ...
          QuizMenu.ChapterSelector(msg, quizMenu, state);
        })
        .catch(collected => {
  ...
  static ChapterSelector(msg, quizMenu, state) {
    const menuEmbed = {
      color: 0x0099ff,
      title: 'SkellyBot Quiz',
    ...
    msg.channel.send({ embed: menuEmbed }).then(() => {
      ...
      msg.channel.awaitMessages(filter, { max: 1, time: 10000, errors: ['time'] })
        .then(collected => {
          ...
          QuizMenu.SectionSelector(msg, quizMenu, state);
        })
        .catch(collected => {
  ...
  static SectionSelector(msg, quizMenu, state) {
